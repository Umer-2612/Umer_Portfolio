                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 3xl:grid-cols-5 gap-3 sm:gap-4 lg:gap-6">
                    {certifications.map((cert, index) => (
                        <motion.a
                            ...
                            whileHover={{ y: -5 }}
                            className="flex flex-col items-center p-3 sm:p-4 lg:p-6 bg-card rounded-xl sm:rounded-2xl border border-border shadow-sm transition-all duration-300 group h-full hover:border-primary/30 hover:shadow-md dark:hover:shadow-[0_0_30px_-10px_hsl(var(--primary)/0.35)]"
                        >
                            {/* Badge Image Area */}
                            ...
                            <div className="text-center flex-grow flex flex-col justify-between w-full">
                                <h3 className="text-[11px] xs:text-xs sm:text-sm lg:text-base 3xl:text-lg font-medium text-foreground mb-1 leading-snug group-hover:text-primary transition-colors">
                                    {cert.title}
                                </h3>

                                <div className="mt-2 sm:mt-3 flex items-center justify-center gap-1 sm:gap-2 text-[9px] sm:text-xs font-medium text-primary opacity-0 group-hover:opacity-100 transition-opacity transform translate-y-1 group-hover:translate-y-0">
